...
def snapshot() -> dict[str, Any]:
    with db_connect() as db:
        ...
        # MT5 history also contains balance/credit operations. A trade journal must
        # expose only BUY (0) and SELL (1) deals, never deposits or withdrawals.
        rows = db.execute(
            "SELECT * FROM deals WHERE account_id=? AND type IN (0,1) "
            "ORDER BY time_msc DESC LIMIT 250",
            (account["id"],),
        ).fetchall()
...
const money=(v,c='$')=>c+Number(v||0).toFixed(2);async function refresh(){const r=await fetch('/api/journal'),j=await r.json();if(!j.connected)return;const a=j.account,s=j.summary,statusEl=document.getElementById('status'),balanceEl=document.getElementById('balance'),equityEl=document.getElementById('equity'),netEl=document.getElementById('net'),tradesEl=document.getElementById('trades'),rowsEl=document.getElementById('rows');statusEl.textContent=`Connected · ${a.provider} · ${a.broker_server} · ${a.broker_login}`;balanceEl.textContent=money(a.balance,a.currency+' ');equityEl.textContent=money(a.equity,a.currency+' ');netEl.textContent=(s.net>=0?'+':'−')+money(Math.abs(s.net),a.currency+' ');netEl.className='value '+(s.net>=0?'good':'bad');tradesEl.textContent=s.closedTrades;rowsEl.textContent='';for(const d of j.deals){const tr=document.createElement('tr');for(const v of [new Date(d.time_msc).toLocaleString(),d.symbol,d.entry,d.volume,d.price,(d.net>=0?'+':'−')+money(Math.abs(d.net),a.currency+' ')]){const td=document.createElement('td');td.textContent=v;tr.appendChild(td)}rowsEl.appendChild(tr)}if(!j.deals.length)rowsEl.innerHTML='<tr><td colspan=6 class=empty>Account linked. No imported trades yet.</td></tr>'}refresh();setInterval(refresh,5000);</script></body></html>"""
